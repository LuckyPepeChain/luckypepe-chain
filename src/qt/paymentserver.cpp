...
const QString BITCOIN_IPC_PREFIX("luckypepe:");
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start luckypepe: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("luckypepe://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'luckypepe://' is not a valid URI. Use 'luckypepe:' instead."),
    ...
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid LuckyPepe address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
