...
static bool GenerateBlock(ChainstateManager& chainman, CBlock& block, uint64_t& max_tries, unsigned int& extra_nonce, uint256& block_hash)
{
    ...
    while (max_tries > 0 && block.nNonce < std::numeric_limits<uint32_t>::max() && !CheckProofOfWork(GetPoWHash(block), block.nBits, chainparams.GetConsensus()) && !ShutdownRequested()) {
        ++block.nNonce;
        --max_tries;
    }
...
static void MiningThread(ChainstateManager* chainman, CTxMemPool* mempool, CScript coinbase_script)
{
    while (g_mining_active.load() && !ShutdownRequested()) {
        try {
            std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(*mempool, Params()).CreateNewBlock(coinbase_script));
            ...
            uint256 block_hash;
            GenerateBlock(*chainman, *pblock, max_tries, extra_nonce, block_hash);
        } catch (...) {
            break;
        }
    }
...
static RPCHelpMan setgenerate()
{
    return RPCHelpMan{"setgenerate",
                "\nSet 'generate' true to turn on mining, false to turn off.\n"
                "Mining is done to a wallet address.\n",
                {
                    {"generate", RPCArg::Type::BOOL, RPCArg::Optional::NO, "Set to true to turn on generation, false to turn off."},
                    {"genproclimit", RPCArg::Type::NUM, /* default */ "1", "Set the number of threads for mining. -1 means all available cores."},
                },
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[1].isNull()) {
        ...
        if (nGenProcLimit < -1) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid genproclimit");
        }
    }
    ...
    if (wallets.empty()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "No wallet available. Use generatetoaddress instead.");
    }
    pwallet = wallets[0];
    LOCK(pwallet->cs_wallet);
    if (!pwallet->CanGetAddresses()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Wallet cannot generate addresses.");
    }
    CTxDestination dest;
    std::string error;
    ...
    std::lock_guard<std::mutex> lock(g_mining_mutex);
    for (int i = 0; i < nGenProcLimit; i++) {
        g_mining_threads.emplace_back(MiningThread, &chainman, const_cast<CTxMemPool*>(&mempool), coinbase_script);
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    result.pushKV("devfundvalue", (int64_t)pblock->vtx[0]->vout[1].nValue);
