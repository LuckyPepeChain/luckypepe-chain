...
static const std::string DEV_FUND_ADDRESS = "lpep1q4aa7j0l5jjac437ammcmm55jf3fqkwlvw3kjqs";
...
bool ReadBlockFromDisk(CBlock& block, const FlatFilePos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (!CheckProofOfWork(GetPoWHash(block), block.nBits, consensusParams))
...
CAmount GetBlockSubsidy(int nHeight, const uint256& hashPrevBlock, const Consensus::Params& consensusParams)
{
    CAmount minReward;
    CAmount maxReward;

    if (nHeight <= 262800) {
        minReward = 1000000LL * COIN;
        maxReward = 3000000LL * COIN;
    } else if (nHeight <= 525600) {
        minReward = 500000LL * COIN;
        maxReward = 1500000LL * COIN;
    } else if (nHeight <= 788400) {
        minReward = 250000LL * COIN;
        maxReward = 750000LL * COIN;
    } else if (nHeight <= 1051200) {
        minReward = 125000LL * COIN;
        maxReward = 375000LL * COIN;
    } else {
        minReward = 25000LL * COIN;
        maxReward = 75000LL * COIN;
    }

    uint64_t seed = hashPrevBlock.GetUint64(0);
    CAmount reward = minReward + (CAmount)(seed % (uint64_t)(maxReward - minReward));
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    hashPrevBlock = pindex->pprev ? pindex->pprev->GetBlockHash() : uint256();
    CAmount blockReward = nFees + GetBlockSubsidy(pindex->nHeight, hashPrevBlock, chainparams.GetConsensus());
    if (block.vtx[0]->GetValueOut() > blockReward) {
        LogPrintf("ERROR: ConnectBlock(): coinbase pays too much (actual=%d vs limit=%d)\n", block.vtx[0]->GetValueOut(), blockReward);
        return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-amount");
    }

    CAmount nSubsidy = GetBlockSubsidy(pindex->nHeight, hashPrevBlock, chainparams.GetConsensus());
    if (nSubsidy > 0) {
        CAmount nExpectedDevFund = nSubsidy * 7 / 100;
        if (block.vtx[0]->vout.size() < 2) {
            LogPrintf("ERROR: ConnectBlock(): coinbase missing dev fund output\n");
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-devfund-missing");
        }
        if (block.vtx[0]->vout[1].nValue != nExpectedDevFund) {
            LogPrintf("ERROR: ConnectBlock(): dev fund amount mismatch (actual=%d vs expected=%d)\n", block.vtx[0]->vout[1].nValue, nExpectedDevFund);
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-devfund-amount");
        }
        CTxDestination devDest = DecodeDestination(DEV_FUND_ADDRESS);
        CScript expectedDevScript = GetScriptForDestination(devDest);
        if (block.vtx[0]->vout[1].scriptPubKey != expectedDevScript) {
            LogPrintf("ERROR: ConnectBlock(): dev fund address mismatch\n");
            return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-devfund-address");
        }
    }
...
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(GetPoWHash(block), block.nBits, consensusParams))
